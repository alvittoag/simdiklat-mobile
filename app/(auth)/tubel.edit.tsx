...
import { ITubelLaporan } from "@/type";

export default function TubelEdit() {
  const queryClient = useQueryClient();
  const { item, semester }: any = useLocalSearchParams();

  const dataParse: ITubelLaporan = JSON.parse(item as string);

  console.log(dataParse.nilai);
  ...
  React.useEffect(() => {
    setDataInput({
      title: dataParse.laporan.nama,
      semester: dataParse.semester.toString(),
      laporan: null as any,
    });

    setSelectValue({
      value: dataParse.laporan.jenis_file.id as any,
      label: dataParse.laporan.jenis_file.nama,
    });
  }, [item]);
  ...
  const mutationAdd = useMutation({
    mutationFn: async (formData: FormData) => {
      return await axiosService.put("/api/tubel/laporan/update", formData, {
        headers: {
    ...
    onError: (error) => {
      ...
      Dialog.show({
        ...
        title: "Gagal",
        textBody: error?.message || "Laporan Tubel Gagal DiUpdate",
        button: "Tutup",
      });
    },
  });
  ...
  const handleAdd = async () => {
    ...
    formData.append("laporan_file_id", dataParse.laporan.id as any);
