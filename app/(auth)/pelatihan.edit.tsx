...
import { IPelatihanUser, IPengajaranUser } from "@/type";

export default function pelatihanEdit() {
  const params = useLocalSearchParams();

  const dataParams: IPengajaranUser = React.useMemo(() => {
    try {
  ...
  React.useEffect(() => {
    if (dataParams) {
      setDataInput({
        kegiatan: dataParams.kegiatan || "",
        instansi: dataParams.instansi || "",
        tahun: dataParams.tahun.toString() || "",
        keterangan: dataParams.keterangan || "",
        materi: dataParams.materi || "",
        expired_sertifikat:
          new Date(dataParams.expired_sertifikat).toLocaleDateString() || "",
        sertifikat_pdf: null as any,
      });
    }
  }, [dataParams]);
  ...
  const mutationEdit = useMutation({
    mutationFn: async (formData: FormData) => {
      return await axiosService.put(
        "/api/data-kompetensi/pengajaran/put",
        formData,
        {
          headers: {
    ...
    onSuccess: () => {
      Dialog.show({
        ...
        title: "Berhasil",
        textBody: "Pelatihan Berhasil Di Update",
        button: "Tutup",
      });
      setDataInput({
        kegiatan: "",
    ...
    onError: (error) => {
      ...
      Dialog.show({
        ...
        title: "Gagal",
        textBody: error?.message || "Pelatihan Gagal Di Update",
        button: "Tutup",
      });
    },
  });
